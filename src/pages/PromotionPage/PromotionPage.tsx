...
import { listPromotions, type BulletinItem } from '@/api/bulletinsApi';
...
export function PromotionPage() {
  const [promotions, setPromotions] = useState<BulletinItem[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');

  const title = useMemo(() => 'Khuyến mãi', []);

  const loadPromotions = async () => {
    try {
      setIsLoading(true);
      setError('');
      const data = await listPromotions(12);
      setPromotions(data);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Không thể tải dữ liệu khuyến mãi.';
      setError(message);
      setPromotions([]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <section className="bg-slate-50 py-8 md:py-12">
      <div className="container mx-auto px-4">
        ...
        {isLoading && <p className="text-sm text-slate-600">Đang tải chương trình khuyến mãi...</p>}

        {!isLoading && error && (
          <Card className="border border-red-200 bg-red-50 shadow-card">
            <CardContent className="flex items-center justify-between gap-4 p-4">
              <p className="text-sm font-medium text-red-700">{error}</p>
              <Button type="button" variant="outline" onClick={loadPromotions}>
                Tải lại
              </Button>
            </CardContent>
          </Card>
        ...
        {!isLoading && !error && promotions.length === 0 && (
          <Card className="border border-slate-300 bg-white shadow-card">
            <CardContent className="p-5 text-sm text-slate-700">
              Hiện chưa có chương trình khuyến mãi nào đang hiển thị.
            </CardContent>
          </Card>
        )}
              ...
              <Card key={item.id} className="border border-slate-300 bg-white shadow-card">
                <CardHeader className="space-y-1">
                  <CardTitle className="line-clamp-2 text-base font-bold text-navy-950">
                    <Link to={`/khuyen-mai/${item.slug}.html`} className="hover:text-primary-700">
                      {item.title}
                    </Link>
                  </CardTitle>
                  <CardDescription>
                ...
                <CardContent className="space-y-2 text-sm text-slate-700">
                  ...
                  <Link to={`/khuyen-mai/${item.slug}.html`} className="inline-block text-sm font-semibold text-primary-700">
                    Xem chi tiết
                  </Link>
                </CardContent>
              </Card>
