...
export function RecruitmentPage() {
  ...
  const title = useMemo(() => 'Tuyển dụng', []);

  const loadRecruitments = async () => {
    try {
      setIsLoading(true);
      setError('');
      const data = await listRecruitments(12);
      setItems(data);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Không thể tải dữ liệu tuyển dụng.';
      setError(message);
      setItems([]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <section className="bg-slate-50 py-8 md:py-12">
      <div className="container mx-auto px-4">
        ...
        {isLoading && <p className="text-sm text-slate-600">Đang tải thông tin tuyển dụng...</p>}

        {!isLoading && error && (
          <Card className="border border-red-200 bg-red-50 shadow-card">
            <CardContent className="flex items-center justify-between gap-4 p-4">
              <p className="text-sm font-medium text-red-700">{error}</p>
              <Button type="button" variant="outline" onClick={loadRecruitments}>
                Tải lại
              </Button>
            </CardContent>
          </Card>
        ...
        {!isLoading && !error && items.length === 0 && (
          <Card className="border border-slate-300 bg-white shadow-card">
            <CardContent className="p-5 text-sm text-slate-700">Hiện chưa có tin tuyển dụng nào đang hiển thị.</CardContent>
          </Card>
        )}
              ...
              <Card key={item.id} className="border border-slate-300 bg-white shadow-card">
                <CardHeader className="space-y-1">
                  <CardTitle className="line-clamp-2 text-base font-bold text-navy-950">
                    <Link to={`/tuyen-dung/${item.slug}.html`} className="hover:text-primary-700">
                      {item.title}
                    </Link>
                  </CardTitle>
                  <CardDescription>
                ...
                <CardContent className="space-y-2 text-sm text-slate-700">
                  ...
                  <Link to={`/tuyen-dung/${item.slug}.html`} className="inline-block text-sm font-semibold text-primary-700">
                    Xem chi tiết
                  </Link>
                </CardContent>
              </Card>
