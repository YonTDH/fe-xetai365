...
type CategoryLevel2SavePayload = Pick<
...
export function AdminCategoryLevel2Page() {
  ...
  const handleSaveCategory = async (nextItem: CategoryLevel2SavePayload) => {
    try {
      setIsSaving(true);
      setError('');

      if (modalState?.mode === 'create') {
        const created = await createAdminVehicleCategoryLevel2(nextItem);
        setModalState(null);
        await loadItems();
        showToast({
          type: 'success',
          message: `Đã thêm danh mục cấp 2 "${created.name}".`,
        });
        return;
      }

      if (!nextItem.id) {
        throw new Error('Thiếu mã danh mục cấp 2.');
      }
          ...
          toolbar={
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                disabled={isLoading || isSaving || isDeletingId !== null || !items.length}
                className="bg-[#135a91] text-white hover:bg-[#0f4b78]"
              >
